...
Every fix attempt operates on a `repomend/fix-<finding-id>` worktree branch.
...
def _fix_worktree_path(finding_id: str) -> Path:
    ...
    """
    return Path(tempfile.gettempdir()) / f"repomend-fix-{finding_id}"


def create_fix_worktree(repo_path: Path, finding_id: str) -> Path:
    """
    Create a git worktree on branch `repomend/fix-{finding_id}`.
    ...
    git_worktree_add(repo_path, fix_path, f"repomend/fix-{finding_id}")
    return fix_path


def cleanup_fix_worktree(repo_path: Path, finding_id: str) -> None:
    """
    Remove the worktree and delete the `repomend/fix-{finding_id}` branch.
...
def fix_worktree_context(
    ...
    Creates a `repomend/fix-{finding_id}` worktree, yields a FixWorktreeHandle,
    ...
    handle = FixWorktreeHandle(
        ...
        branch=f"repomend/fix-{finding_id}",
    )
    try:
        yield handle
    finally:
