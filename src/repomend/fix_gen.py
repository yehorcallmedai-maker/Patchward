...
if TYPE_CHECKING:
    from repomend.config import RepomendConfig
...
def _execute_fix_tool(
    ...
    from repomend.hooks import DeniedToolCallError, check_tool_call  # noqa: PLC0415
    try:
...
class FixGenSubagent:
    ...
    async def apply_fix(
        ...
        Args:
            ...
            worktree_path: Path to the fix worktree (repomend/fix-<id> branch).
        ...
        # The scan runs in a temp worktree (repomend-scan-{uuid}).
        # Fix-Gen must work in a different temp worktree (repomend-fix-{id}).
        ...
        if fp_abs.is_absolute():
            ...
            for i, part in enumerate(parts):
                if part.startswith("repomend-scan-") or part.startswith("repomend-fix-"):
                    rel_start = i + 1
                    break
