...
# assumption: repomend.toml in cwd | test: test_config.py,
...
from repomend.scanner import run_all_scanners
from repomend.subagent import ScannerSubagent
from repomend.verifier import Verifier
from repomend.worktree import require_git_version, worktree_context
from repomend import tracing
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"repomend {_VERSION}")
        raise typer.Exit()
...
@app.command()
def version() -> None:
    """Print repomend version and exit."""
    from repomend import __version__
    typer.echo(f"repomend {__version__}")
...
def scan(
    ...
    config: Optional[Path] = typer.Option(
        None, "--config", help="Path to repomend.toml"
    ),
) -> None:
    ...
    with tracing.span(
        "repomend.scan", repo=str(repo_path), rules=cfg.semgrep_rules
    ):
        ...
        try:
            ...
            with worktree_context(repo_path) as scan_path:
                with tracing.span(
                    "repomend.scan_all", repo=str(scan_path)
                ):
            ...
            with tracing.span(
                "repomend.store_findings", count=len(findings)
            ):
                for f in findings:
                    insert_finding(
                        conn,
            ...
            typer.echo(
                f"[repomend] Scan complete. Findings: {len(findings)}"
            )
            ...
            if cfg.anthropic_api_key and findings:
                try:
                    with tracing.span(
                        "repomend.subagent.triage", repo=str(repo_path)
                    ):
                    ...
                    typer.echo(
                        f"\n[repomend] Triage summary: {triage.summary}"
                    )
            ...
            typer.echo(
                f"[repomend] ERROR: unexpected failure: {exc}", err=True
            )
...
def fix(
    ...
    model: Optional[str] = typer.Option(
        ...
        help=(
            ...
            "Takes precedence over [models].fix_model in repomend.toml. "
    ...
    if not cfg.anthropic_api_key:
        typer.echo(
            "[repomend] ERROR: ANTHROPIC_API_KEY is required for "
            "the fix command.",
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    with tracing.span(
        "repomend.fix", repo=str(repo_path), rules=cfg.semgrep_rules
    ):
        conn = open_db(cfg.db_path)
        repo_id = get_or_create_repo(conn, str(repo_path))
        run_id = create_run(
        ...
        try:
            with worktree_context(repo_path) as scan_path:
                with tracing.span(
                    "repomend.fix.scan_all", repo=str(scan_path)
                ):
                    sarif_runs = run_all_scanners(
                        scan_path, cfg.semgrep_rules
                    )
            ...
            with tracing.span(
                "repomend.fix.store_findings", count=len(findings)
            ):
                for f in findings:
                    insert_finding(
                        conn,
            ...
            if not findings:
                typer.echo("[repomend] No findings — nothing to fix.")
                return
            ...
            if skipped_count:
                typer.echo(
                    f"[repomend] Skipping {skipped_count} finding(s) in "
            ...
            if not findings:
                typer.echo(
                    "[repomend] No actionable findings after filtering."
                )
                return
            ...
            for finding in findings:
                ...
                # Scan runs in a temp dir (repomend-scan-{uuid}); fix and verifier
                # need the relative path from the repo root (e.g. "checkdmarc/spf.py").
                _fp = Path(file_path)
                rel_file_path = file_path
                if _fp.is_absolute():
                    for _i, _part in enumerate(_fp.parts):
                        if _part.startswith("repomend-scan-") or _part.startswith("repomend-fix-"):
...
def batch(
    ...
    Reads ``[[repos]]`` from ``repomend.toml``, processes each
    ...
    if not github_token:
        typer.echo(
            "[repomend] ERROR: GITHUB_TOKEN is required for "
            "the batch command.",
            err=True,
        )
        raise typer.Exit(code=1)
    ...
    if not cfg.repos:
        typer.echo(
            "[repomend] ERROR: No [[repos]] entries found in "
            "repomend.toml. Add at least one [[repos]] entry or "
    ...
    typer.echo(f"[repomend] Batch log: {run_log.path}")
