...
def setup_tracing(langfuse_host: str, enabled: bool) -> None:
    ...
    if not enabled:
        _tracer = trace.get_tracer("patchward.noop")
        return
    ...
    if not public_key or not secret_key:
        import typer
        typer.echo(
            "[patchward] WARNING: LANGFUSE_PUBLIC_KEY / LANGFUSE_SECRET_KEY not set. "
    ...
    try:
        ...
        _provider = TracerProvider(
            resource=Resource.create({"service.name": "patchward"}),
        )
        # SimpleSpanProcessor: synchronous export — correct for short-lived CLI processes.
        _provider.add_span_processor(SimpleSpanProcessor(exporter))
        trace.set_tracer_provider(_provider)
        _tracer = trace.get_tracer("patchward")

    except Exception as exc:
        import typer
        typer.echo(f"[patchward] WARNING: tracing setup failed (scan continues): {exc}", err=True)
        _tracer = trace.get_tracer("patchward.noop")
