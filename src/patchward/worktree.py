...
Every scan operates on a `patchward/scan-<id>` worktree branch.
...
This module contains only scan-specific naming logic (patchward/scan-<id>).
...
def _worktree_path(scan_id: str) -> Path:
    ...
    """
    return Path(tempfile.gettempdir()) / f"patchward-scan-{scan_id}"


def create_worktree(repo_path: Path, scan_id: str) -> Path:
    """
    Create a git worktree on branch `patchward/scan-{scan_id}`.
    ...
    git_worktree_add(repo_path, scan_path, f"patchward/scan-{scan_id}")
    return scan_path


def cleanup_worktree(repo_path: Path, scan_id: str) -> None:
    """
    Remove the worktree and delete the `patchward/scan-{scan_id}` branch.
    ...
    # KS-TRACE: C-P2-05, C-P2-08, AC-P2-06
    """
    git_worktree_remove(repo_path, _worktree_path(scan_id), f"patchward/scan-{scan_id}")
...
def worktree_context(repo_path: Path) -> Generator[Path, None, None]:
    ...
    No `patchward/scan-*` branch or worktree directory is ever leaked.
    ...
    try:
        ...
        # A leaked patchward/scan-* branch on the caller's repo is a violation
