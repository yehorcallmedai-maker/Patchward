...
# assumption: patchward.toml lives in cwd or explicit path
# test: test_config.py
...
def _build_repos(
    ...
    for idx, entry in enumerate(repos_raw):
        ...
        if "path" not in entry:
            typer.echo(
                f"[patchward] ERROR: [[repos]] entry {idx} is missing"
                " required field 'path'.",
                err=True,
            )
            raise SystemExit(1)
...
def load_config(
    ...
    path = config_path or Path("patchward.toml")

    if not path.exists():
        typer.echo(
            f"[patchward] ERROR: config file not found: {path}",
            err=True,
        )
        raise SystemExit(1)
    ...
    try:
        ...
        typer.echo(
            f"[patchward] ERROR: could not parse config: {exc}",
            err=True,
        )
        raise SystemExit(1)

    section = raw.get("patchward", {})
...
def validate_github_config(cfg: RepomendConfig) -> None:
    ...
    if missing:
        fields = ", ".join(missing)
        msg_lines = [
            "[patchward] ERROR: patchward.toml is missing required",
            f"[github] fields: {fields}.",
            "Add to patchward.toml:",
            "  [github]",
        ]
        for f in missing:
