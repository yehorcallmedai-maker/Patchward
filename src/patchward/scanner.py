...
from patchward.sarif import SARIFNormalizer, SARIFRun

if TYPE_CHECKING:
    from patchward.docker_sandbox import DockerSandbox, NetworkPolicy
...
def _require_tool(name: str, install_hint: str) -> None:
    if shutil.which(name) is None:
        typer.echo(
            f"[patchward] ERROR: '{name}' not found on PATH. {install_hint}",
            err=True,
        )
...
def _run_subprocess(
    ...
    *,
    ...
    try:
        ...
        typer.echo(f"[patchward] ERROR: '{cmd[0]}' timed out after {timeout}s", err=True)
        raise typer.Exit(code=1)
    except Exception as exc:
        typer.echo(f"[patchward] ERROR: '{cmd[0]}' failed to launch: {exc}", err=True)
        raise typer.Exit(code=1)

    if result.returncode not in acceptable_exit_codes:
        typer.echo(
            f"[patchward] ERROR: '{cmd[0]}' exited {result.returncode}:\n{result.stderr[:500]}",
            err=True,
        )
        raise typer.Exit(code=1)
...
def _parse_json(raw: str, tool_name: str) -> dict | list:
    try:
        ...
        typer.echo(
            f"[patchward] ERROR: could not parse {tool_name} output as JSON: {exc}",
            err=True,
        )
        raise typer.Exit(code=1)
...
def run_semgrep(
    ...
    if sandbox is not None:
        from patchward.docker_sandbox import NetworkPolicy
...
def run_pip_audit(
    ...
    if not req_file.exists():
        typer.echo(
            f"[patchward] SKIP: pip-audit — no requirements.txt in {repo_path}",
            err=True,
        )
...
def run_eslint(
    ...
    if not is_node_repo(repo_path):
        typer.echo(
            f"[patchward] SKIP: eslint — no package.json in {repo_path}",
            err=True,
        )
...
def run_npm_audit(
    ...
    if not is_node_repo(repo_path):
        typer.echo(
            f"[patchward] SKIP: npm-audit — no package.json in {repo_path}",
            err=True,
        )
...
def run_trivy(
    ...
    if sandbox is not None:
        ...
        if shutil.which("trivy") is None:
            typer.echo(
                "[patchward] SKIP: trivy — not found on PATH"
    ...
    if not result.stdout.strip():
        typer.echo(
            "[patchward] SKIP: trivy — no scannable targets (no Dockerfile or IaC files found)",
            err=True,
        )
        return SARIFRun(tool_name="trivy")
...
def run_osv_scanner(
    ...
    if sandbox is not None:
        ...
        if shutil.which("osv-scanner") is None:
            typer.echo(
                "[patchward] SKIP: osv-scanner — not found on PATH"
    ...
    if not result.stdout.strip():
        typer.echo(
            "[patchward] SKIP: osv-scanner — no scannable targets found",
            err=True,
        )
        return SARIFRun(tool_name="osv-scanner")
